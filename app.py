...
v5.4: timestamp BR | v5.5: retry audio | v5.6: ACK+conversa | v5.7: fix 403 Backblaze com client-token
"""
...
def claude(mensagem):
    ...
    payload = {"model": "claude-haiku-4-5-20251001", "max_tokens": 1024, "system": SYSTEM_PROMPT, "messages": [{"role": "user", "content": mensagem}]}
...
def groq_transcrever(audio_url):
    # Z-API armazena áudios no Backblaze — requer client-token para download
    download_headers = {
    ...
    for tentativa in range(3):
        try:
            ...
            with urllib.request.urlopen(req, timeout=30) as r:
                audio_bytes = r.read()
            if audio_bytes:
                print(f"Download áudio OK: {len(audio_bytes)} bytes")
                break
    ...
    if not audio_bytes:
        raise Exception(f"Áudio indisponível (último erro: {last_error}): {audio_url[:80]}")
...
def make_enviar(tipo, titulo, detalhes, recebido_em):
    payload = {"text": titulo, "tipo": tipo, "detalhes": detalhes, "fonte": "whatsapp", "recebido_em": recebido_em}
...
def zapi_enviar(telefone, mensagem):
    ...
    req = urllib.request.Request(f"{ZAPI_BASE}/send-text", data=json.dumps(payload).encode(), headers=headers, method="POST")
    try:
...
def webhook():
    ...
    try:
        ...
        DESTINOS = {"tarefa": "Asana", "agenda": "Google Calendar", "nota": "Notion", "email": "Outlook", "planilha": "Excel"}
        if tipo in DESTINOS and MAKE_WEBHOOK_URL:
            destino = DESTINOS[tipo]
            try:
                make_enviar(tipo, titulo, detalhes, recebido_em)
                zapi_enviar(telefone, f"✅ {tipo.capitalize()} registrada no {destino}!\n📌 {titulo}\n🕐 {recebido_em}")
            except Exception as e:
                print(f"[{recebido_em}] Erro Make: {e}")
                zapi_enviar(telefone, f"⚠️ Não consegui enviar para o {destino}.\nO que pedi: {titulo}\nTente novamente ou acesse o {destino} diretamente.")
        else:
